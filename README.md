...
```sh
mise run setup          # deps + build + web bundle (required before the first dev boot)
mise run dev            # boot the dev server (home ~/.grove, http://127.0.0.1:7777; GROVE_DEV_PORT overrides)
